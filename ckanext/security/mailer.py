...
from ckan.lib.mailer import get_reset_link_body
...
def send_reset_link(user):
    ...
    tk.mail_user(user, subject, body)


def _build_footer_content(extra_vars):
    custom_path = tk.config.get('ckanext.security.brute_force_footer_path')
    if (custom_path and os.path.exists(custom_path)):
        ...
        return '\n\n' + tk.render(footer_path, extra_vars)
...
def notify_lockout(user, lockout_timeout):
    extra_vars = {
        ...
        'user_name': user.name,
        'password_reset_url':
            tk.config.get('ckan.site_url').rstrip('/') + '/user/login',
        'lockout_mins': lockout_timeout // 60,
    }

    subject = tk.render('security/emails/lockout_subject.txt', extra_vars)

    subject = subject.split('\n')[0]  # Make sure we only use the first line

    body = tk.render('security/emails/lockout_mail.txt', extra_vars)\
